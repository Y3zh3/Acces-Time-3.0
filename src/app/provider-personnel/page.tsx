...
export default function ProviderPersonnelManagement() {
  ...
  // Cargar personal de proveedores
  const loadAccessLogs = async () => {
    try {
      setLoading(true)
      const response = await fetch(`/api/provider-personnel`)
      if (!response.ok) throw new Error('Error al cargar personal')
      const data = await response.json()
      
      // Cargar todo el personal de proveedores
      setAccessLogs(data.personnel || [])
    } catch (error) {
      console.error('Error:', error)
      toast({
        variant: 'destructive',
        title: 'Error',
        description: 'No se pudo cargar el personal de proveedores'
      })
    } finally {
      setLoading(false)
    }
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const dataToSend = {
        ...editFormData,
        photoPath: newPhoto || undefined  // Solo enviar si hay nueva foto
      }

      const response = await fetch(`/api/provider-personnel?id=${editingPerson.id}`, {
        method: 'PUT',
      ...
      toast({
        ...
        description: `${editFormData.fullName} actualizado correctamente`
      })
  ...
  const exportToPDF = () => {
    ...
    doc.text('INFORME DE PERSONAL DE PROVEEDORES', 105, 15, { align: 'center' });
    ...
    autoTable(doc, {
      head: [['DNI', 'Nombre', 'Empresa', 'Cargo', 'Teléfono', 'Estado', 'Entrada']],
      body: tableData,
      startY: 70,
    ...
    const fileName = `Informe_Personal_Proveedores_${format(new Date(), 'dd-MM-yyyy')}.pdf`;
    doc.save(fileName);
  ...
  const exportToExcel = () => {
    ...
    const resumenData = [
      ['INFORME DE PERSONAL DE PROVEEDORES'],
      [''],
    ...
    const dataToExport = dataToFilter.map(log => ({
      ...
      'CARGO': log.position?.toUpperCase() || '-',
      'TELÉFONO': log.phone || '-',
    ...
    wsDetalle['!cols'] = [
      { wch: 12 }, { wch: 30 }, { wch: 25 }, { wch: 20 }, { wch: 15 }, { wch: 15 }, { wch: 18 }, { wch: 18 }, { wch: 18 }, { wch: 18 },
    ];
    XLSX.utils.book_append_sheet(workbook, wsDetalle, 'Personal de Proveedores');
    
    const fileName = `Informe_Personal_Proveedores_${format(new Date(), 'dd-MM-yyyy')}.xlsx`;
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Gestión del personal de empresas proveedoras.</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card className="bg-gradient-to-br from-purple-50 to-purple-100 border-purple-200">
          <CardContent className="pt-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-purple-700 font-semibold">INACTIVOS</p>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow key={log.id}>
                ...
                <TableCell className="text-sm text-muted-foreground">{log.position || '-'}</TableCell>
            ...
            {!loading && filteredAccessLogs.length === 0 && (
              <TableRow><TableCell colSpan={11} className="text-center py-10 text-muted-foreground italic">No hay personal registrado.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </Card>
